...
class TestDefaultAndFallback(AcceleratorTestBase):
    """The observable ``use_accelerator`` contract on the real path."""

    def _write_and_read(self, table, janitor):
    ...
    def test_default_enables_accelerator(self, default_table, janitor):
        """Default construction (no flag) turns the accelerator on and works."""
        assert default_table._accelerator_client is not None
        self._write_and_read(default_table, janitor)

    def test_explicit_true_enables_accelerator(self, accel_table, janitor):
        """``use_accelerator=True`` runs on the accelerated path end-to-end."""
        assert accel_table._accelerator_client is not None
        self._write_and_read(accel_table, janitor)

    def test_explicit_false_uses_native(self, native_table, janitor):
        ...
        self._write_and_read(native_table, janitor)

    def test_emulator_with_explicit_true_raises(self, monkeypatch):
    ...
    def test_emulator_default_disables_with_warning(self, monkeypatch):
