...
class TestAcceleratorHealthMonitor:
    @staticmethod
    ...
    def test_probe_reads_status_off_the_wire(self, tmp_path, status, expected):
        server = self._start_server(tmp_path, status=status)
        client = self._make_client(tmp_path)
        try:
            monitor = self._make_monitor(client, AcceleratorBreaker())
            assert monitor._probe() is expected
        finally:
            client.close()
            server.stop()

    def test_probe_sends_the_accelerator_token(self, tmp_path):
        server = self._start_server(tmp_path)
        client = self._make_client(tmp_path)
        try:
            monitor = self._make_monitor(client, AcceleratorBreaker())
            assert monitor._probe() is True
            assert server.last_metadata["x-accelerator-token"] == _AUTH_SECRET
        finally:
            client.close()
            server.stop()

    def test_probe_times_out_on_a_starved_daemon(self, tmp_path):
        server = self._start_server(tmp_path)
        server.delay = 1.0
        client = self._make_client(tmp_path)
        try:
            ...
            assert monitor._probe() is False
            assert time.monotonic() - started < 0.9
        finally:
            client.close()
            server.stop()

    def test_probe_reports_unimplemented_distinctly(self, tmp_path):
        server = self._start_server(tmp_path, serve_health=False)
        client = self._make_client(tmp_path)
        try:
            monitor = self._make_monitor(client, AcceleratorBreaker())
            assert monitor._probe() is None
        finally:
            client.close()
            server.stop()

    def test_probe_fails_when_nothing_is_listening(self, tmp_path):
        client = self._make_client(tmp_path)
        try:
    ...
    def test_loop_degrades_then_recovers(self, tmp_path):
        server = self._start_server(tmp_path)
        client = self._make_client(tmp_path)
        ...
        try:
            monitor.start()
            assert self._wait_for(lambda: server.request_count >= 2)
            assert not breaker.bypass()
            server.status = ServingStatus.NOT_SERVING
            assert self._wait_for(lambda: breaker.is_degraded)
            assert breaker.bypass()
            assert not breaker.is_tripped
            server.status = ServingStatus.SERVING
            assert self._wait_for(lambda: not breaker.is_degraded)
            assert not breaker.bypass()
        finally:
            monitor.close()
            client.close()
    ...
    def test_loop_stops_against_a_daemon_without_health(self, tmp_path):
        server = self._start_server(tmp_path, serve_health=False)
        client = self._make_client(tmp_path)
        breaker = AcceleratorBreaker()
        ...
        try:
            monitor.start()
            assert self._wait_for(lambda: monitor._task.done())
            assert not breaker.bypass()
        finally:
            monitor.close()
            client.close()
    ...
    def test_loop_stops_once_the_breaker_is_tripped(self, tmp_path):
        server = self._start_server(tmp_path)
        client = self._make_client(tmp_path)
        breaker = AcceleratorBreaker()
        monitor = self._make_monitor(client, breaker)
        try:
            monitor.start()
            assert self._wait_for(lambda: server.request_count >= 1)
            breaker.trip()
            assert self._wait_for(lambda: monitor._task.done())
        finally:
            monitor.close()
    ...
    def test_close_stops_polling(self, tmp_path):
        server = self._start_server(tmp_path)
        client = self._make_client(tmp_path)
        monitor = self._make_monitor(client, AcceleratorBreaker())
        try:
            ...
            assert self._wait_for(lambda: server.request_count >= 1)
            monitor.close()
            CrossSync._Sync_Impl.sleep(0.2)
    ...
    def test_close_leaves_a_degraded_flag_set(self, tmp_path):
